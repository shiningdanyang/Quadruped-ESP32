...
void setLegPWM(leg &_leg)
{
  ...
  uint16_t param1 = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.alpha, _leg.hal.mid.alpha, _leg.hal.trim.alpha, _leg.hal.ratio.alpha, _leg.inverse.alpha)));
  uint16_t param2 = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.beta,  _leg.hal.mid.beta,  _leg.hal.trim.beta,  _leg.hal.ratio.beta,  _leg.inverse.beta )));
  uint16_t param3 = angleToPulse(limitServoAngle(getHALAngle(_leg.angle.gamma, _leg.hal.mid.gamma, _leg.hal.trim.gamma, _leg.hal.ratio.gamma, _leg.inverse.gamma)));
  ESP32_ISR_Servos.setPulseWidth(esp32pwmServos[l+0], param1);
  ESP32_ISR_Servos.setPulseWidth(esp32pwmServos[l+1], param2);
  ESP32_ISR_Servos.setPulseWidth(esp32pwmServos[l+2], param3);
